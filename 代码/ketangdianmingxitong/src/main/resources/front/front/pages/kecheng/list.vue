<template>
	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        <view class="search-form round">
				<input v-model="searchForm.kechengName" type="text" placeholder="课程名称"></input>
		</view>
	...
	<scroll-view scroll-x="true" class="tabView"
		...
		<view :style='categoryName===item.codeIndex?sactiveItem:sitem' class="tab"
			  v-for="(item,index) in kechengTypesList" :key="index"
...
<view class="uni-product-list" :style='{"borderRadius":0,"backgroundColor":"rgba(224, 224, 224, 1)"}'>
    <view
        ...
        <view class="uni-product-price" v-if="product.kechengNewMoney != null"
              :style='{"fontSize":"24rpx","lineHeight":"60rpx","color":"#fff","textAlign":"center"}'>
            <text class="">￥{{product.kechengNewMoney}}</text>
        </view>
        <view style="display: flex;justify-content: space-between;">
            <text v-if="isAuth('kecheng','修改')" class="cuIcon-edit"
                  @click.stop="onUpdateTap(product.id)">修改</text>
            <text v-if="isAuth('kecheng','删除')" class="cuIcon-delete"
            ...
            v-if="isAuth('kecheng','新增')" class="add-btn" @click="onAddTap()">新增</button>
	...
	export default {
		...
		async onShow() {
            ...
            _this.btnColor = _this.btnColor.sort(() => {
	   		...
	   		//当前表的 课程类型 字段 字典表查询方法
		  	let kechengTypes = await this.$api.page('dictionary', {
			 	 page: 1,
			  	limit: 100,
		  		dicCode:'kecheng_types'
		  	});
		  	this.kechengTypesList = kechengTypes.data.list;
	   		//当前表的 班级 字段 字典表查询方法
		  	let banjiTypes = await this.$api.page('dictionary', {
			 	 page: 1,
			  	limit: 100,
		  		dicCode:'banji_types'
		  	});
		  	this.banjiTypesList = banjiTypes.data.list;
		...
		methods: {
				...
				let params = {
					...
					limit: mescroll.size
							...
							if (this.categoryName != 0) {
                                params.kechengTypes = this.categoryName
                            }

                let res = await this.$api.list(`kecheng`, params);
			...
			onDeleteTap(id) {
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('kecheng', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
			...
			async search() {
				...
				if (this.searchForm.kechengUuidNumber) {
                    searchForm['kechengUuidNumber'] = this.searchForm.kechengUuidNumber
                }
				if (this.searchForm.kechengName) {
                    searchForm['kechengName'] = this.searchForm.kechengName
                }
				if (this.searchForm.kechengName) {
					searchForm['kechengName'] = this.searchForm.kechengName
				}

                let res = await this.$api.list(`kecheng`, searchForm);
				// 如果是第一页数据置空
